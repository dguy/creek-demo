...
application {
    mainModule.set("creek.demo.service")
    mainClass.set("io.github.dguy.creek.demo.service.ServiceMain")
}
...
val buildAppImage = tasks.create("buildAppImage", DockerBuildImage::class) {
    ...
    images.add("ghcr.io/dguy/${rootProject.name}-${project.name}:latest")
    images.add("ghcr.io/dguy/${rootProject.name}-${project.name}:${project.version}")
}
